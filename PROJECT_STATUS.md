...
Full suite: **1345 passing, 0 failing**. All Pinnacle tests pass (`tests/test_pinnacle_value_model.py` — 41 tests incl. 5 Phase 18B required cases + 13 Phase 18C diagnostics tests; `tests/test_phase15_official_picks.py` — 21 tests incl. 6 Pinnacle-gate tests).
...
**Completed**: Pinnacle + alt-line diagnostics only (Phase 18C) — per-group `PINNACLE_GROUP` debug logs, `LINE_FRAGMENTATION` debug logs, single-line `PINNACLE_SUMMARY` INFO per run, `pinnacle_diagnostics` in scan results + `run_summary.json`, `--debug` CLI flag. No logic/threshold changes. Full suite **1345 passed, 0 failed**. Pinnacle not in feed → market-median fallback is production path; official picks require a `pinnacle` key in the feed.
