...
Full suite: **1332 passing, 0 failing**. All Pinnacle tests pass (`tests/test_pinnacle_value_model.py` — 28 tests incl. 5 Phase 18B required cases; `tests/test_phase15_official_picks.py` — 21 tests incl. 6 Pinnacle-gate tests).
...
**Completed**: Pinnacle required for official picks (Phase 18B) — `REQUIRE_PINNACLE_FOR_OFFICIAL=True` by default, Pinnacle gate in `classify_recommendation`, fallback opportunities displayed but never official, same-line guard, `PINNACLE_CHECK`/`OFFICIAL_BLOCKED_REQUIRE_PINNACLE` logs, 11 new tests. Full suite **1332 passed, 0 failed**. Pinnacle not in feed → market-median fallback is production path; official picks require a `pinnacle` key in the feed.
