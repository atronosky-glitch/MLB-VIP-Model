...
def init_db(db_path: str | None = None) -> None:
    ...
    # Prediction-market execution layer, Stage 4 (human-approved live
    # execution): every table below is part of the real-money path, but
    # no order is ever placed without a valid, unexpired, single-use
    # ExecutionAuthorization checked at submission time. See
    # src/execution/live/. place_order/cancel_order remain
    # NotImplemented on every provider -- these tables exist regardless
    # of whether live trading is ever actually enabled.
    conn.execute("""
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_prepared_live_orders_status ON prepared_live_orders(status)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_prepared_live_orders_fingerprint ON prepared_live_orders(fingerprint)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_execution_authorizations_status ON execution_authorizations(status)")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_execution_authorizations_prepared "
        "ON execution_authorizations(prepared_order_id)"
    )
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_live_submission_attempts_state ON live_submission_attempts(state)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_live_orders_approval ON live_orders(approval_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_live_orders_status ON live_orders(status)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_live_fills_order ON live_fills(live_order_id)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_live_positions_status ON live_positions(status)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_live_positions_event ON live_positions(event_id)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_live_execution_events_approval ON live_execution_events(approval_id)")
