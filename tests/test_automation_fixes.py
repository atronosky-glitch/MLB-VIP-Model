...
class TestMLBExchangePropsScheduling:
    """src.worker._check_and_schedule_mlb_exchange_props /
    _run_mlb_exchange_props_scan (2026-09-10, operator-enabled) —
    Kalshi/Novig/Polymarket/ProphetX props, deliberately isolated from
    the regular MLB props scan above (own job type, own cadence, own
    last-fetch tracking) so enabling this never collides with it."""

    def test_no_games_creates_no_exchange_props_job(self, db_conn):
        with patch.object(worker, "_mlb_game_times_from_db", return_value=[]):
            worker._check_and_schedule_mlb_exchange_props(db_conn)
        count = db_conn.execute(
            "SELECT COUNT(*) AS c FROM scheduled_jobs WHERE job_type = 'mlb-exchange-props-scan'"
        ).fetchone()["c"]
        assert count == 0

    def test_game_inside_window_schedules_exchange_props_job(self, db_conn):
        now = worker._now_local().replace(hour=12, minute=0, second=0, microsecond=0)
        game_time = now + timedelta(hours=2)  # inside the 3h exchange-props window
        with patch.object(worker, "_mlb_game_times_from_db", return_value=[game_time]), \
             patch.object(worker, "_now_local", return_value=now):
            worker._check_and_schedule_mlb_exchange_props(db_conn)
        row = db_conn.execute(
            "SELECT job_type FROM scheduled_jobs WHERE job_type = 'mlb-exchange-props-scan'"
        ).fetchone()
        assert row is not None

    def test_does_not_duplicate_queued_job(self, db_conn):
        now = worker._now_local().replace(hour=12, minute=0, second=0, microsecond=0)
        game_time = now + timedelta(hours=2)
        with patch.object(worker, "_mlb_game_times_from_db", return_value=[game_time]), \
             patch.object(worker, "_now_local", return_value=now):
    ...
    def test_scheduling_this_does_not_also_queue_the_regular_props_job(self, db_conn):
        """Real bug shape this guards against: the two checks share
        _mlb_game_times_from_db but must write to distinct job_type rows,
        never accidentally queuing (or being satisfied by) the other's job."""
        now = worker._now_local().replace(hour=12, minute=0, second=0, microsecond=0)
        game_time = now + timedelta(hours=2)
        with patch.object(worker, "_mlb_game_times_from_db", return_value=[game_time]), \
             patch.object(worker, "_now_local", return_value=now):
            worker._check_and_schedule_mlb_exchange_props(db_conn)
        regular_count = db_conn.execute(
            "SELECT COUNT(*) AS c FROM scheduled_jobs WHERE job_type = 'mlb-props-scan'"
        ).fetchone()["c"]
    ...
    def test_job_type_registered_in_dispatch(self):
        import inspect
        source = inspect.getsource(worker._execute_job)
        assert '"mlb-exchange-props-scan"' in source

    def test_run_mlb_exchange_props_scan_fetches_and_persists_rows(self, db_conn):
        """_run_mlb_exchange_props_scan must call fetch_mlb_exchange_props
        and persist whatever it returns directly (save_player_prop_batch),
        bypassing run_pipeline entirely -- unlike the regular MLB props
        job, this must never touch the EV-picks recommendation engine."""
        fake_row = {
            "event_id": "evt-1", "odd_id": "m-evt-1-kalshi", "sportsbook": "kalshi",
            "player_id": "p1", "player_name": "Test Player", "team_id": "", "team_name": "",
            "market_type": "batting_homeRuns_ou", "market_group_key": "evt-1|p1|0.5",
            "side": "OVER", "line": 0.5, "price": 130, "decimal_odds": 2.30,
            "is_alt_line": 0, "available": 1, "validation_status": "VALID",
            "mapping_confidence": "HIGH", "mapping_method": "roster", "validation_reason": "OK",
            "captured_at": worker._now_local().isoformat(),
        }
        with patch("src.sports.mlb.fetch_mlb_exchange_props", return_value=([fake_row], [])):
            result = worker._run_mlb_exchange_props_scan(db_conn, config=None)

        assert result["status"] == "success"
        assert result["rows_captured"] == 1
        saved = db_conn.execute(
            "SELECT sportsbook FROM player_prop_odds WHERE event_id = 'evt-1'"
        ).fetchone()
        assert saved is not None
        assert saved["sportsbook"] == "kalshi"

    def test_run_mlb_exchange_props_scan_handles_no_rows(self, db_conn):
        with patch("src.sports.mlb.fetch_mlb_exchange_props", return_value=([], [])):
            result = worker._run_mlb_exchange_props_scan(db_conn, config=None)
        assert result["status"] == "success"
