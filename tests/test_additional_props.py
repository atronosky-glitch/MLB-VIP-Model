...
class TestWalksAllowed:
    ...
    def test_yn_parsed(self):
        result = parse_player_props(walks_event)
        yn = [r for r in result.odds_rows if r["market_type"] == "pitching_basesOnBalls_yn"]
        assert len(yn) == 2  # 1 player x 2 books (Yes side only)

    def test_yn_yes_side(self):
        result = parse_player_props(walks_event)
        yn = [r for r in result.odds_rows if r["market_type"] == "pitching_basesOnBalls_yn"]
        assert all(r["side"] == "YES" for r in yn)

    def test_yn_no_side_excluded(self):
        result = parse_player_props(walks_event)
        yn_no = [r for r in result.audit_rows
                 if r.get("market_type") == "pitching_basesOnBalls_yn" and r.get("side") == "NO"]
        assert len(yn_no) == 1
        assert yn_no[0]["excluded"] == 1

    def test_yn_line_is_none(self):
        result = parse_player_props(walks_event)
        yn = [r for r in result.odds_rows if r["market_type"] == "pitching_basesOnBalls_yn"]
        assert all(r["line"] is None for r in yn)
    ...
    def test_ou_yn_keys_disjoint(self):
        result = parse_player_props(walks_event)
        ...
        yn_keys = {r["market_group_key"] for r in result.odds_rows
                   if r["market_type"] == "pitching_basesOnBalls_yn"}
    ...
    def test_yn_analysis_valid(self):
        yes = {
            "book1": {"price": -350, "decimal_odds": 1.2857, "validation_status": "VALID"},
            "book2": {"price": -375, "decimal_odds": 1.2667, "validation_status": "VALID"},
            "book3": {"price": -360, "decimal_odds": 1.2778, "validation_status": "VALID"},
            "book4": {"price": -340, "decimal_odds": 1.2941, "validation_status": "VALID"},
        }
        result = analyze_yn_group("test", yes)
        assert result["market_quality"] == MARKET_QUALITY_VALID
        assert result["n_books"] == 4

    def test_yn_insufficient_books(self):
        yes = {
            "book1": {"price": -350, "decimal_odds": 1.2857, "validation_status": "VALID"},
            "book2": {"price": -375, "decimal_odds": 1.2667, "validation_status": "VALID"},
        }
    ...
    def test_registry_config(self):
        ...
        assert PITCHER_WALKS_ALLOWED.market_type_yn == "pitching_basesOnBalls_yn"
        assert PITCHER_WALKS_ALLOWED.display_name == "Pitcher Walks Allowed"
        assert PITCHER_WALKS_ALLOWED.short_label == "BB"
        assert PITCHER_WALKS_ALLOWED.supports_yn is True
    ...
    def test_registry_match_yn(self):
        assert match_yn_market("pitching_basesOnBalls-X-game-yn-yes") is PITCHER_WALKS_ALLOWED
        assert match_yn_market("pitching_basesOnBalls-X-game-yn-no") is PITCHER_WALKS_ALLOWED

    def test_registry_by_type(self):
        assert get_market_by_ou_type("pitching_basesOnBalls_ou") is PITCHER_WALKS_ALLOWED
        assert get_market_by_yn_type("pitching_basesOnBalls_yn") is PITCHER_WALKS_ALLOWED
...
class TestEarnedRuns:

    def test_ou_parsed_correct_count(self):
        result = parse_player_props(earned_runs_event)
        ou = [r for r in result.odds_rows if r["market_type"] == "pitching_earnedRuns_ou"]
        assert len(ou) == 10  # 1 player x 2 sides x 5 books

    def test_yn_parsed(self):
        result = parse_player_props(earned_runs_event)
        yn = [r for r in result.odds_rows if r["market_type"] == "pitching_earnedRuns_yn"]
        assert len(yn) == 2  # 1 player x 2 books (Yes side only)
    ...
    def test_yn_no_side_excluded(self):
        result = parse_player_props(earned_runs_event)
        yn_no = [r for r in result.audit_rows
                 if r.get("market_type") == "pitching_earnedRuns_yn" and r.get("side") == "NO"]
        assert len(yn_no) == 1
        assert yn_no[0]["excluded"] == 1
    ...
    def test_player_name(self):
        result = parse_player_props(earned_runs_event)
        names = {r["player_name"] for r in result.odds_rows}
        assert "Framber Valdez" in names

    def test_group_key_market_type(self):
        result = parse_player_props(earned_runs_event)
        for r in result.odds_rows:
            assert "pitching_earnedRuns" in r["market_group_key"]

    def test_ou_yn_keys_disjoint(self):
        result = parse_player_props(earned_runs_event)
        ou_keys = {r["market_group_key"] for r in result.odds_rows
                   if r["market_type"] == "pitching_earnedRuns_ou"}
        yn_keys = {r["market_group_key"] for r in result.odds_rows
                   if r["market_type"] == "pitching_earnedRuns_yn"}
        assert ou_keys.isdisjoint(yn_keys)

    def test_ou_analysis_valid(self):
        over = {f"b{i}": {"price": -110, "decimal_odds": 1.9091, "line": 2.5} for i in range(5)}
        under = {f"b{i}": {"price": -110, "decimal_odds": 1.9091, "line": 2.5} for i in range(5)}
        result = analyze_prop_group("test", over, under)
        assert result["market_quality"] == MARKET_QUALITY_VALID

    def test_yn_analysis_valid(self):
        yes = {
            "book1": {"price": -250, "decimal_odds": 1.4, "validation_status": "VALID"},
            "book2": {"price": -275, "decimal_odds": 1.3636, "validation_status": "VALID"},
            "book3": {"price": -260, "decimal_odds": 1.3846, "validation_status": "VALID"},
            "book4": {"price": -240, "decimal_odds": 1.4167, "validation_status": "VALID"},
        }
        result = analyze_yn_group("test", yes)
        assert result["market_quality"] == MARKET_QUALITY_VALID
        assert result["n_books"] == 4
    ...
    def test_registry_config(self):
        assert PITCHER_EARNED_RUNS.cli_name == "earned_runs"
        assert PITCHER_EARNED_RUNS.odd_id_stat_prefix == "pitching_earnedRuns"
        assert PITCHER_EARNED_RUNS.market_type_ou == "pitching_earnedRuns_ou"
        assert PITCHER_EARNED_RUNS.market_type_yn == "pitching_earnedRuns_yn"
        assert PITCHER_EARNED_RUNS.display_name == "Pitcher Earned Runs"
        assert PITCHER_EARNED_RUNS.short_label == "ER"
        assert PITCHER_EARNED_RUNS.supports_yn is True

    def test_registry_match_ou(self):
        assert match_ou_market("pitching_earnedRuns-X-game-ou-over") is PITCHER_EARNED_RUNS
        assert match_ou_market("pitching_earnedRuns-X-game-ou-under") is PITCHER_EARNED_RUNS

    def test_registry_match_yn(self):
        assert match_yn_market("pitching_earnedRuns-X-game-yn-yes") is PITCHER_EARNED_RUNS
        assert match_yn_market("pitching_earnedRuns-X-game-yn-no") is PITCHER_EARNED_RUNS

    def test_registry_by_type(self):
        assert get_market_by_ou_type("pitching_earnedRuns_ou") is PITCHER_EARNED_RUNS
        assert get_market_by_yn_type("pitching_earnedRuns_yn") is PITCHER_EARNED_RUNS

    def test_registry_by_cli(self):
        assert get_market_by_cli_name("earned_runs") is PITCHER_EARNED_RUNS

    def test_missing_side_excluded(self):
        under = {"b1": {"price": -110, "decimal_odds": 1.9091, "line": 2.5}}
        result = analyze_prop_group("test", {}, under)
        assert result["market_quality"] == MARKET_QUALITY_EXCLUDED

    def test_negative_ev(self):
        over = {f"b{i}": {"price": -120, "decimal_odds": 1.8333, "line": 2.5} for i in range(5)}
        under = {f"b{i}": {"price": 100, "decimal_odds": 2.0, "line": 2.5} for i in range(5)}
        result = analyze_prop_group("test", over, under)
        for b in result["books"]:
            assert b["ev_pct"] < 0

    def test_cross_market_isolation(self):
        """ER and hits don't mix."""
        combined = {
            "eventID": "COMBO",
            "odds": {
                "pitching_earnedRuns-P_1_MLB-game-ou-over": {
                    "playerID": "P_1_MLB", "playerNames": {"full": "T"},
                    "marketName": "T Earned Runs Over/Under",
                    "byBookmaker": {f"b{i}": {"odds": -110, "overUnder": 2.5, "available": True} for i in range(5)},
                },
                "pitching_earnedRuns-P_1_MLB-game-ou-under": {
                    "playerID": "P_1_MLB", "playerNames": {"full": "T"},
                    "marketName": "T Earned Runs Over/Under",
                    "byBookmaker": {f"b{i}": {"odds": -110, "overUnder": 2.5, "available": True} for i in range(5)},
                },
        ...
        result = parse_player_props(combined)
        er_keys = {r["market_group_key"] for r in result.odds_rows
                   if r["market_type"] == "pitching_earnedRuns_ou"}
        hits_keys = {r["market_group_key"] for r in result.odds_rows
                     if r["market_type"] == "pitching_hits_ou"}
        assert er_keys.isdisjoint(hits_keys)


# ====================================================================
# Cross-market: all 6 markets don't mix
# ====================================================================

class TestAllMarketsIsolation:

    def test_all_six_markets_independent(self):
        """All 6 supported markets for same player produce separate group keys."""
        event = {
            "eventID": "MEGA",
            "odds": {},
        }
        markets = [
            ...
            ("pitching_outs", "Outs Recorded Over/Under", 17.5),
            ("pitching_hits", "Hits Allowed Over/Under", 5.5),
            ("pitching_basesOnBalls", "Walks Over/Under", 1.5),
            ("pitching_earnedRuns", "Earned Runs Over/Under", 2.5),
        ]
...
class TestRegistryCompleteness:
    ...
    def test_all_ou_type_lookups(self):
        ...
        assert get_market_by_ou_type("pitching_earnedRuns_ou") is not None

    def test_yn_type_lookups(self):
        assert get_market_by_yn_type("pitching_strikeouts_yn") is not None
        assert get_market_by_yn_type("pitching_basesOnBalls_yn") is not None
        assert get_market_by_yn_type("pitching_earnedRuns_yn") is not None
