...
with st.container(border=True):
    _hcol1, _hcol2 = st.columns([6, 2])
    with _hcol1:
        st.markdown("# :material/sports_baseball: MLB VIP Model")
        st.caption("Model-driven MLB prop recommendations · Shadow mode — no wagers are placed.")
    with _hcol2:
        st.markdown(":gray[**Mode**]")
        if shadow and shadow.shadow_mode:
            st.markdown(":orange[**SHADOW**]")
        else:
            st.markdown(":red[**LIVE**]")

header_cols = st.columns(4, border=True)
with header_cols[0]:
...
with tabs[0]:
    ...
    st.caption("Every recommendation generated by the model today, filterable. Official picks are frozen records; Discovery and Research picks are for threshold calibration.")

    import pandas as pd
    recs_today = _load_recs(db_path, "today")
    if not recs_today:
        st.info("No recommendations generated today yet. Run the pipeline from the Run & Operations tab.")
    else:
        def _pick_edge(r: dict[str, Any]) -> float | None:
            """Edge metric: EV% for OU markets, implied-prob advantage for YN markets."""
            if r.get("market_form") == "yn":
                return r.get("yn_implied_prob_adv")
            return r.get("ev_pct")

        official_today = [r for r in recs_today if r.get("recommendation_tier") == "OFFICIAL_TRACKED"]
        discovery_today = [r for r in recs_today if r.get("recommendation_tier") == "DISCOVERY_TRACKED"]
        research_today = [r for r in recs_today if r.get("recommendation_tier") == "RESEARCH_ONLY"]

        kpi_cols = st.columns(4, border=True)
        kpi_cols[0].metric("Total Picks", len(recs_today))
        kpi_cols[1].metric("Official", len(official_today))
        kpi_cols[2].metric("Discovery", len(discovery_today))
        kpi_cols[3].metric("Research", len(research_today))

        if not official_today:
            st.caption(
                "No official picks qualified today — "
                "see the Official Picks tab for why."
            )

        # Filters
        filter_cols = st.columns(4)
        with filter_cols[0]:
            markets = sorted(set(r.get("market_type", "") for r in recs_today if r.get("market_type")))
            sel_market = st.selectbox("Market", ["All"] + markets, key="picks_filter_market")
        with filter_cols[1]:
            sel_tier = st.multiselect(
                "Tier",
                ["OFFICIAL_TRACKED", "DISCOVERY_TRACKED", "RESEARCH_ONLY"],
                default=["OFFICIAL_TRACKED", "DISCOVERY_TRACKED", "RESEARCH_ONLY"],
                key="picks_filter_tier",
            )
        with filter_cols[2]:
            min_ev = st.number_input(
                "Min Edge (%)",
                min_value=-50.0, max_value=50.0, value=0.0, step=0.5,
                key="picks_filter_min_ev",
            )
        with filter_cols[3]:
            query = st.text_input("Search player / matchup", key="picks_filter_query")

        filtered = []
        for r in recs_today:
            tier = r.get("recommendation_tier") or "RESEARCH_ONLY"
            if sel_market != "All" and r.get("market_type") != sel_market:
                continue
            if tier not in sel_tier:
                continue
            ...
            if query.strip():
                q = query.strip().lower()
                hay = f"{r.get('player_name', '')} {r.get('matchup', '')}".lower()
                if q not in hay:
                    continue
            filtered.append(r)

        filtered.sort(key=lambda r: -(_pick_edge(r) or 0))
        ...
        for r in filtered:
            ...
            table_data.append({
                ...
                "Model Score": round(r["model_score"], 1) if r.get("model_score") is not None else None,
                "Tier": (r.get("recommendation_tier") or "RESEARCH_ONLY").replace("_TRACKED", "").replace("_ONLY", "").title(),
                "Matchup": r.get("matchup", ""),
                "Start": (r.get("event_start_time") or "")[:16],
                "Status": r.get("event_status", ""),
            })

        if table_data:
            st.dataframe(pd.DataFrame(table_data), use_container_width=True, hide_index=True)
            st.caption(f"Showing {len(filtered)} of {len(recs_today)} picks · Edge is EV% (O/U) or price advantage in pp (Yes/No).")
        else:
