...
with tabs[0]:
    ...
    st.caption("Every recommendation generated by the model today. Official picks are frozen and ranked; Discovery and Research picks support calibration.")
    ...
    if not recs_today:
        ...
        if not official_today:
            st.caption(
                ":gray[No official picks qualified today — "
        ...
        with filter_cols[1]:
            sel_tier = st.segmented_control(
                "Tier",
                ["All", "Official", "Discovery", "Research"],
                default="All",
        ...
        tier_to_key = {"All": None, "Official": "OFFICIAL_TRACKED", "Discovery": "DISCOVERY_TRACKED", "Research": "RESEARCH_ONLY"}
        sel_tier_key = tier_to_key.get(sel_tier)
        ...
        for r in recs_today:
            ...
            if sel_market != "All" and r.get("market_type") != sel_market:
                continue
            if sel_tier_key and tier != sel_tier_key:
                continue
            edge = _pick_edge(r) or 0.0
            if edge < min_ev:
                continue
        ...
        for r in filtered:
            ...
            table_data.append({
                ...
                "Tier": _tier_badge(r.get("recommendation_tier")),
        ...
        if table_data:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Model Score": st.column_config.NumberColumn("Model Score", format="%.1f"),
                    "Tier": st.column_config.MarkdownColumn("Tier"),
            ...
            st.caption(f"Showing {len(filtered)} of {len(recs_today)} picks · Edge is EV% (O/U) or price advantage in pp (Yes/No). Model Score is a quality metric, not a win probability.")
        else:
