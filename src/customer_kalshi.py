"""Per-customer Kalshi Auto-Bet connection (2026-09-23).

Lets a logged-in customer (see src/customer_accounts.py) connect their
OWN Kalshi account -- API Key ID + an RSA private key (PEM), the exact
two credentials src/execution/kalshi.py's KalshiProvider actually
requires, confirmed by reading that provider directly rather than
guessed. Verification uses the real provider's health_check() (a
harmless authenticated GET, /portfolio/balance -- never an order).
Credentials are encrypted at rest (src/credential_encryption.py, the
SAME Fernet system already used for Polymarket -- one well-designed
encryption system, not a duplicate) and decrypted only in memory, only
when a caller needs them for a verify or execution attempt -- never
returned from any function here, never logged.

This module is a deliberate structural mirror of
src/customer_polymarket.py -- same conventions, same defaults, same
safety posture -- adapted only for Kalshi's different credential shape
(RSA/PEM instead of Ed25519/base64) and its own, fully independent
per-platform risk settings (see that module's docstring for why Kalshi
and Polymarket are NOT collapsed into one shared table).
...
requires risk settings to already be configured. live_execution (PAPER
vs LIVE) is a SEPARATE flag, also default OFF: even once both are true
for a customer, src/execution/customer_autobet.py still re-checks the
SAME global config gates (config.live_trading_enabled /
config.kalshi_live_enabled) the existing single-operator live-execution
path already enforces -- a customer's own settings are necessary, never
sufficient, for a real order.
"""
...
# Deliberately identical shape to src.customer_polymarket's defaults --
# only the two platforms' own customer-configured values ever diverge.
...
def get_status(conn, account_id: str) -> KalshiAccountStatus:
    """Never includes decrypted credentials -- only a masked display
    string built from the stored fingerprint."""
    row = conn.execute(
        "SELECT * FROM customer_kalshi_accounts WHERE account_id = ?", (account_id,)
    ).fetchone()
    if row is None:
        return KalshiAccountStatus(False, None, None, None, None, None, False, False)
    d = dict(row)
    fp = d.get("api_key_id_fingerprint")
    return KalshiAccountStatus(
        connected=bool(d["kalshi_connected"]),
...
def verify_credentials(api_key_id: str, private_key_pem: str) -> tuple[bool, str]:
    """Authenticate against the REAL Kalshi client with a harmless,
    read-only, authenticated request (portfolio balance) -- never
    places an order. Returns (ok, message); message is always safe to
    show the customer directly -- never the raw provider exception."""
    if not api_key_id or not private_key_pem:
        return False, "API Key ID and Private Key are both required."
    try:
        provider = KalshiProvider(api_key_id=api_key_id, private_key_pem=private_key_pem)
    except CredentialLoadError:
        return False, "That Private Key doesn't look valid -- check that you pasted the whole PEM value."

    try:
        result = provider.health_check()
    except Exception:
        logger.exception("Kalshi verify_credentials health_check raised unexpectedly")
        return False, "Could not reach Kalshi right now -- please try again in a moment."

    if not result.ok:
        logger.info("Kalshi credential verification failed for a customer (detail withheld from customer-facing message)")
        return False, "Kalshi rejected those credentials. Double-check your API Key ID and Private Key."
    return True, "Connected successfully."


def connect_account(conn, account_id: str, api_key_id: str, private_key_pem: str) -> tuple[bool, str]:
    ...
    ok, message = verify_credentials(api_key_id, private_key_pem)
    ...
    try:
        ...
        encrypted_private_key = encrypt_secret(private_key_pem)
    except CredentialEncryptionError:
        logger.exception("Could not encrypt customer Kalshi credentials")
        return False, "Could not save your credentials right now -- please try again in a moment."

    api_fp = fingerprint_secret(api_key_id)
    cred_fp = credential_set_fingerprint(api_key_id, private_key_pem)

    conn.execute(
        """INSERT INTO customer_kalshi_accounts (
               account_id, kalshi_connected, encrypted_api_key_id, encrypted_private_key,
    ...
    conn.commit()
    return True, "Kalshi account connected."
...
def _upsert_verify_only(conn, account_id: str, now: str, status: str, error: str) -> None:
    ...
    existing = conn.execute(
        "SELECT account_id FROM customer_kalshi_accounts WHERE account_id = ?", (account_id,)
    ).fetchone()
    if existing:
        conn.execute(
            """UPDATE customer_kalshi_accounts
        ...
        conn.execute(
            """INSERT INTO customer_kalshi_accounts (
                   account_id, kalshi_connected, last_verified_at, last_verify_status,
...
def disconnect_account(conn, account_id: str) -> None:
    ...
    now = datetime.now(timezone.utc).isoformat()
    conn.execute(
        """UPDATE customer_kalshi_accounts SET
               kalshi_connected = 0, encrypted_api_key_id = NULL, encrypted_private_key = NULL,
...
def get_decrypted_credentials(conn, account_id: str) -> tuple[str, str] | None:
    ...
    long enough to construct a KalshiProvider."""
    row = conn.execute(
        "SELECT kalshi_connected, encrypted_api_key_id, encrypted_private_key "
        "FROM customer_kalshi_accounts WHERE account_id = ?", (account_id,),
    ).fetchone()
    if row is None:
        return None
    d = dict(row)
    if not d["kalshi_connected"] or not d["encrypted_api_key_id"] or not d["encrypted_private_key"]:
        return None
    api_key_id = decrypt_secret(d["encrypted_api_key_id"])
    private_key_pem = decrypt_secret(d["encrypted_private_key"])
...
def enable_autobet(conn, account_id: str) -> tuple[bool, str]:
    ...
    if row is None or not dict(row)["kalshi_connected"]:
        return False, "Connect your Kalshi account first."
    d = dict(row)
    ...
    conn.execute(
        "UPDATE customer_kalshi_accounts SET autobet_enabled = 1, updated_at = ? WHERE account_id = ?",
...
def disable_autobet(conn, account_id: str) -> None:
    ...
    conn.execute(
        "UPDATE customer_kalshi_accounts SET autobet_enabled = 0, updated_at = ? WHERE account_id = ?",
        (datetime.now(timezone.utc).isoformat(), account_id),
    )
    conn.commit()
...
def set_live_execution(conn, account_id: str, live: bool) -> None:
    ...
    apply on top."""
    conn.execute(
        "UPDATE customer_kalshi_accounts SET live_execution = ?, updated_at = ? WHERE account_id = ?",
...
def save_risk_settings(conn, account_id: str, **settings) -> None:
    ...
    if existing is None:
        raise ValueError("connect a Kalshi account before saving risk settings")

    set_clause = ", ".join(f"{f} = ?" for f in settings)
    values = [settings[f] for f in settings]
    conn.execute(
        f"UPDATE customer_kalshi_accounts SET {set_clause}, updated_at = ? WHERE account_id = ?",
