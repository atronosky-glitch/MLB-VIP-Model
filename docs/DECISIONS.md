...
- **Date**: 2026-08-23
- **Decision**: Rebuilt `src/pinnacle_feed.py` to support MLB, NFL, and WNBA (previously MLB-only, despite `REQUIRE_PINNACLE_FOR_OFFICIAL` already applying to every league — meaning NFL/WNBA O/U picks could structurally never become Official). Also extended Pinnacle coverage to full-game markets (moneyline/spread-or-runline/total) for all 3 leagues, not just player props — parsed from each event's "Game" period (`num_0`/`description=="Game"`) in the same pinnapi.com fixtures payload. Game markets are matched into the analysis pipeline via the existing `player_id="GAME"` sentinel (already set generically by `src/player_prop_parser.py` and `src/odds_api_game_parser.py`), so no new analysis engine was needed — `src/market_analysis.py::analyze_two_way_market` remains confirmed dead code. New multi-league config lives in `src/prop_config.py`: `PINNACLE_SPORT_ID_BY_LEAGUE` (MLB=6, NFL=5, WNBA=3 — WNBA shares pinnapi's "Basketball" sport_id with NBA and other basketball leagues, filtered by real `league_name`), `PINNACLE_LEAGUE_NAME_BY_LEAGUE`, `PINNACLE_PROP_UNITS_BY_LEAGUE`/`PINNACLE_PROP_SUFFIXES_BY_LEAGUE` (per-league prop unit<->market_type maps and label-suffix formats, live-verified — see `docs/MARKET_CAPABILITY.md`), and `PINNACLE_GAME_MARKET_TYPES_BY_LEAGUE` (MLB uses run-line naming `game_runline_ou`; NFL/WNBA use `game_spread_ou`).
- **Reason**: Operator's explicit, repeated instruction: "we wnat pinnacle to be used for all 3 leagues yes also we want moneylines and over unders included... i want official picks from all 3 leagues." The operator personally verified Pinnacle carries the relevant markets and pushed back when told Pinnacle was "unreachable" — correctly identifying that was a framing error, not a real limitation: Pinnacle (via `src/pinnacle_feed.py`/pinnapi.com) is a real, separate, working data source from SportsGameOdds/Odds-API, it was just never extended past MLB in code.
- **Consequence**: Also swapped MLB's own registered `batter_hits` market for `batter_home_runs` in `src/mlb_props_parser.py` — Pinnacle's real MLB feed prices home runs, not hits, so `batter_hits` could have real Odds-API book depth but could never pass Gate 9's Pinnacle-match requirement for Official status; `batter_home_runs` has a real path to Official even with thinner raw book depth (mitigated further by the LOO book-count floor, see the entry below). A real rate-limit bug was caught and fixed before shipping: fetching props and game odds as two independent calls would hit the shared 10-second module-level rate limiter back-to-back and silently drop the second call almost every time in real usage; fixed by caching the raw pinnapi payload once per league (`PinnacleFeedClient._get_raw_payload`) and parsing both props and game odds from the same cached payload. Live-verified 2026-08-23 against the real API with the new pinnapi trial key: MLB 26 props / 227 game-market entries across 16 games; NFL 0 props (real — no specials posted yet this far before its season opener, confirmed via full `special_category` breakdown, not a bug) / 368 game-market entries across 16 games; WNBA 82 props / 60 game-market entries across 4 games — end-to-end matching and injection into synthetic O/U groups confirmed working for both a real WNBA player prop and a real WNBA game total market.
...
- **Consequence**: Per-book `pinnacle_fair_prob`/`pinnacle_ev`/`pinnacle_prob_edge`/`pinnacle_approved` are computed when Pinnacle is present. Scanner `Pin` column, verbose block, and reference-source header make the reference strategy visible in output.

## Invalid API keys fail fast instead of retrying (2026-08-01)

- **Decision**: Authentication failures are fatal and non-retryable. `_is_auth_failure(status, text)` in `src/api_client.py` returns True for HTTP 401/403 and for any HTTP ≥ 400 response whose body carries an auth marker (`invalid api key`, `unauthorized`, `authentication failed`, etc.); such responses raise `APIKeyError` immediately (`logger.critical`, key masked via `_mask_key`). Only transient statuses (429, 502, 503, 504, plain 500, timeouts) retry with backoff. The correct env var is `SPORTSODDS_API_KEY` (key sent via `x-api-key` header).
- **Reason**: SportsGameOdds reports invalid keys as HTTP 500 with body `"Internal Server Error: Invalid API key"`, which was inside the retry set → the client retried ~6 times (45s+ wasted per run) before failing. A bad key is a configuration error that retry can never fix; failing fast surfaces the real problem immediately. Note: the "retrying in 45s - attempt #6" text seen in old Render logs exists nowhere in the repo — it was a stale log, not a code path.
- **Consequence**: `_request_with_retry` raises `APIKeyError` for auth failures, retries otherwise. Import-time key diagnostic logs a masked key. Default CLI log level raised to INFO in `src/daily_pipeline.py`/`src/player_prop_scanner.py` so the `PINNACLE_SUMMARY` INFO line is visible without `--debug`.

## `fallback_lean` counts groups that reached the lean stage (2026-08-01)

- **Decision**: In `_accumulate_pinnacle_summary` (`src/player_prop_scanner.py`), `fallback_lean` increments only when the LOO median fallback was used AND `rejection_reason != "insufficient_comparison_books"`. Previously it counted every group analyzed with the fallback reference, including groups rejected for too few comparison books (first live run: 2121 vs 779 groups that actually reached the lean stage).
- **Reason**: The label implies lean opportunities. A group that never reached the lean stage (insufficient comparison books) is not a fallback lean, and inflating the number by ~9× misled operators.
- **Consequence**: `PINNACLE_SUMMARY fallback_lean` now matches groups that reached the lean stage (equal to `pinnacle_missing` while Pinnacle is absent from the feed). All other counters unchanged. Diagnostics-only — no pick-logic change.

## Pinnacle + alt-line diagnostics only — Phase 18C (2026-07-31)

- **Decision**: Add a logging/metadata observability layer for the Pinnacle value model and alt-line fragmentation — nothing that changes pick logic, thresholds, or book filtering. Per-group structured `diagnostics` dicts plus three log channels: `PINNACLE_GROUP` (DEBUG, one line per group), `LINE_FRAGMENTATION` (DEBUG, one line per player+market listing every exact line present, book names, whether Pinnacle is on that line vs other lines, O/U side counts), and `PINNACLE_SUMMARY` (INFO, one compact line per run with counters for exact-match, reference-used, missing, line-mismatch, one-side, model-disabled, insufficient-books, EV/edge threshold failures, fallback, official-approved). Diagnostics are also persisted: `diagnostics` on each analysis result and `pinnacle_diagnostics` on the scan result / `run_summary.json`. Both scanner and daily pipeline get a `--debug` flag; production stays quiet by default (per-group logs are DEBUG-level).
- **Reason**: After 18B, official picks depend on Pinnacle approval but the feed has no `pinnacle` book — operators could not see *why* each group was rejected or how lines are fragmented across books. The 18A/18B thresholds (`MIN_PINNACLE_EV`, `MIN_PINNACLE_PROB_EDGE`, `REQUIRE_PINNACLE_FOR_OFFICIAL`, `MIN_COMPARISON_BOOKS`) and fallback behavior were explicitly frozen for this phase.
- **Consequence**: Every group analysis carries a machine-readable `diagnostics` dict (`rejection_reason` ∈ approved|missing_pinnacle|pinnacle_line_mismatch|pinnacle_missing_opposite_side|pinnacle_model_disabled|insufficient_comparison_books|ev_threshold_failed|prob_edge_threshold_failed|no_positive_edge). The `PINNACLE_SUMMARY` line gives a one-glance read of an entire run. Running `python -m src.player_prop_scanner --debug` (or pipeline `--debug`) emits the full per-group + fragmentation detail. Full suite **1345 passed, 0 failed**.

## Pinnacle required for official picks — Phase 18B (2026-07-31)

- **Decision**: `REQUIRE_PINNACLE_FOR_OFFICIAL` now defaults to `True`. Official picks require Pinnacle approval: both Pinnacle sides at the exact same player/market/side/line, EV ≥ `MIN_PINNACLE_EV`, probability edge ≥ `MIN_PINNACLE_PROB_EDGE`. The gate lives in `classify_recommendation` (O/U only; YN single-sided markets are never gated because they have no Pinnacle counterpart). Fallback/market-median opportunities are still displayed but are always `is_official=False`; the earlier Phase 18A strict-mode behavior of suppressing them (`best_ev=None`/`NO_BET`) was removed.
- **Reason**: Fallback consensus (LOO median) is not a sharp enough reference to justify an "official" pick once Pinnacle is adopted as the standard. Suppressing fallback output entirely hides legitimate research leads; marking them non-official preserves visibility without overstating confidence. `rec_eligible` stays quality-based; the new `is_official` flag is the official-pick gate, propagated opportunity → rec dict → `classify_recommendation`.
- **Consequence**: Per-book `is_official` + group `pinnacle_found`/`pinnacle_reference_used`/`pinnacle_book`/`pinnacle_over_price`/`pinnacle_under_price`/`official_count` added to `analyze_prop_group`. Same-line guard logs `PINNACLE_LINE_FRAGMENTATION` instead of silently merging different lines. Debug logs `PINNACLE_CHECK` (per group) and `OFFICIAL_BLOCKED_REQUIRE_PINNACLE` (reason=`missing_pinnacle`|`pinnacle_threshold_failed`). In production (no Pinnacle in feed) fallback rows are stored as DISCOVERY_TRACKED/RESEARCH_ONLY and never OFFICIAL_TRACKED.

## Recommendation-save error is distinguishable from an intentional duplicate skip

- **Date**: 2026-08-25
- **Decision**: `save_recommendation()` now returns a real `.status` (`saved`/`duplicate`/`error`) via a new `save_recommendation_result()` / `SaveRecommendationResult`, instead of a bare `id | None` where both a duplicate and a genuine persistence failure collapsed to the same `None`. `database/db_manager.py::_persist_recommendation_evidence()` now casts `pinnacle_found`/`pinnacle_reference_used`/`pinnacle_approved` through a new `_bool_to_int_or_none()` helper before binding them as SQL parameters.
- **Reason**: A real 3-day production outage (zero recommendations saved 2026-08-22 through 2026-08-24) traced live to a raw Python `bool` bound against an `integer`-typed PostgreSQL column in an UPDATE — SQLite accepts this silently, PostgreSQL raises `psycopg2.errors.DatatypeMismatch`. The prior bare `except Exception: return None` swallowed this with zero signal, and the existing test suite could not have caught it since it only runs against SQLite (`tests/test_phase17b_postgres.py`'s own docstring says so). Found by live reproduction against the real production schema, not by assumption, per operator direction.
- **Consequence**: `daily_pipeline.py::_stage_freeze` now counts `saved`/`duplicates`/`save_errors` as three independently-tracked values (`duplicates` was previously inferred by subtraction) and surfaces save errors with the failing recommendation's identifiers into the pipeline's error list. `save_recommendation()` is kept as a backward-compatible thin wrapper over the new function for any caller that only needs the old `id | None` contract.

## NFL schedule discovery falls back to The Odds API on a real SportsGameOdds 429

- **Date**: 2026-08-25
- **Decision**: `src/worker.py::_discover_nfl_game_times()` catches `requests.exceptions.HTTPError` specifically; on `status_code == 429` it falls back to `OddsAPIClient.get_events(sport_key=ODDS_API_SPORT_KEY)` (The Odds API's free `/events` endpoint, reusing `src/sports/nfl.py`'s existing sport-key constant). Any other exception still degrades to `[]`, same as before the change.
- **Reason**: The prior code caught every exception indiscriminately and returned `[]`, and `_check_and_schedule_nfl()` returns immediately on an empty game-times list — meaning a real, ongoing SportsGameOdds 429 silenced NFL scheduling entirely (confirmed live: no NFL job had been queued since 2026-08-22). Mirrors the exact `except requests.exceptions.HTTPError` / `status == 429` pattern `daily_pipeline.py` already uses for its own game-odds 429 fallback, and reuses the same already-integrated Odds-API provider WNBA's schedule discovery uses — operator explicitly asked not to build new architecture.
- **Consequence**: The Odds API's `/events` endpoint takes no time-window parameters, so the fallback returns the full remaining season (272 games, live-verified) rather than a near-term slate; `nfl_should_run_daily_scan`/`nfl_should_run_pregame_check` already filter by real calendar date/proximity, so this doesn't cause incorrect scheduling, only a larger in-memory list than the normal 9-day-windowed SGO path. Live-verified: real SGO 429 in production, fallback correctly recovered 272 real game times.
