...
- ~~Multi-league support~~ Architecture done 2026-08-19 (NFL added, WNBA unblocked). ~~Automated post-game settlement~~ done 2026-08-20, shared across MLB/NFL/WNBA (`src/game_settlement.py`). ~~Reliable CLV observation scheduling~~ line-movement-aware CLV done 2026-08-20. ~~Website~~ multi-sport pick lifecycle done 2026-08-20 (`src/customer_view.py`) — remaining: run NFL/WNBA live on a schedule, full `control_panel.py` multi-league pass.
...
11. ~~Implement scheduled snapshots and settlement (MLB).~~ Settlement is done and shared across all 3 leagues (`src/game_settlement.py`, 2026-08-20); scheduled snapshots for CLV are also working (line-movement-aware as of 2026-08-20). Remaining: none of this runs on a schedule for NFL/WNBA yet (see item 14/16 below).
12. Implement alt-line scanning.
13. ~~**Operator decision needed**: WNBA data access.~~ Resolved 2026-08-19 (The Odds API, free tier). **New operator decision, only if a scheduled props cadence is wanted**: the $30/mo paid tier — see `docs/MARKET_CAPABILITY.md` → WNBA cost note.
14. Run NFL live in production: wire a schedule/cron (weekly cadence, distinct from MLB's daily one), verify Pinnacle coverage or accept LOO-only, verify the Render deployment end to end.
15. ~~Customer-facing multi-league UI (`src/customer_view.py`).~~ Done 2026-08-20 — MLB/NFL/WNBA, filters, fair odds/confidence/CLV fields, Performance Dashboard with breakdowns. A full `control_panel.py` (internal admin) multi-league pass is still open.
16. Run WNBA live in production: wire a schedule/cron for both game odds and (opt-in, cost-aware) player props into `render.yaml`/`src/scheduler.py`.
