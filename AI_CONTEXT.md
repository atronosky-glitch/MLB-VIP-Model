...
16. **(2026-08-22)**: SportsGameOdds quota exhaustion for MLB/NFL, resolved with a tested fallback to The Odds API for game markets — done, see `docs/SESSION_HANDOFF.md`'s 2026-08-21/22 entry. ~~Player props for that fallback are still out of scope~~ **Done later the same day**: MLB/NFL now have real supplemental player props via The Odds API too — not a fallback, a genuine second data source running on its own (deliberately narrower than WNBA's) schedule regardless of SportsGameOdds's health. `src/player_identity.py` generalized cleanly to MLB/NFL's ESPN rosters (only its grouped roster-response shape needed handling). 4 markets registered per league from a live liquidity check, all reusing each league's existing settlement contract. See `docs/MARKET_CAPABILITY.md`'s "Supplemental player props" sections.
